...
# Win streak/losing streak, last x games wins, home/away performance
...
mlb <- data.frame(game.id = seq(1,nrow(raw)),
                  ...
                  away.ball.30 = as.integer(NA), home.ball.30 = as.integer(NA),
                  away.wins.20 = as.integer(NA), home.wins.20 = as.integer(NA)
                  )
mlb <- mlb %>%
  filter(season>2013,season!=2020,playoff=="") %>%
...
mlb <- mlb %>%
  mutate(stadium = replace(stadium,season>2016&stadium=="ATL","ATL2"),
         ...
         stadium = replace(stadium,season==2021&date>="2021-06-01"&stadium=="TOR","TOR3"),
...
for(i in c(1:30)){
  ...
  for(j in c(1:nrow(temp))){
    num <- temp$game.id[j]
    if(j>31){
      mlb$away.ball.30[num] <- mean(w[c((j-15):(j-1))])
      mlb$home.ball.30[num] <- mean(x[c((j-15):(j-1))])
    }
    else{
      mlb$away.ball.30[num] <- mean(w[c(1:(j-1))])
      mlb$home.ball.30[num] <- mean(x[c(1:(j-1))])
    }
  }
}
...
y <- vector()
for(i in c(1:30)){
  temp <- filter(mlb, away.team == mlb.teams[i])
  y <- abs(temp$winner-1)
  y <- as.integer(y)
  for(j in c(1:nrow(temp))){
    if(temp$away.team[j] == mlb.teams[i]){
      num <- temp$game.id[j]
      if(j>21)
        mlb$away.wins.20[num] <- sum(y[c((j-20):(j-1))])
      else
        mlb$away.wins.20[num] <- sum(y[c(1:j)])
    }
  }
  temp <- filter(mlb, home.team == mlb.teams[i])
  y <- temp$winner
  ...
  for(j in c(1:nrow(temp))){
    if(temp$home.team[j] == mlb.teams[i]){
      num <- temp$game.id[j]
      if(j>21)
        mlb$home.wins.20[num] <- sum(y[c((j-20):(j-1))])
      else
        mlb$home.wins.20[num] <- sum(y[c(1:j)])
    }
  }
}
...
loc <- data.frame(matrix(ncol = 3, nrow = 34))
x <- c("team", "long","lat")
colnames(loc) <- x
loc$stadium <- c("ARI","ATL","ATL2","BAL","BOS","CHC","CHW","CIN","CLE","COL","DET","HOU","KC","LAA","LAD","MIA","MIL","MIN","NYM","NYY","OAK","PHI","PIT","SD","SEA","SF","STL","TB","TEX","TEX2","TOR","TOR2","TOR3","WAS")
loc$long <- c(-112.0675133,-84.3899584,-84.4689525,-76.6227698,-71.0981172,-87.6558899,-87.6346931,-84.5071904,-81.6858458,-104.994519,-83.0493191,-95.3564899,-94.4808422,-117.8834119,-118.2407581,-80.2203884,-87.9719091,-93.2790431,-73.8466345,-73.9268563,-122.2017169,-75.1679395,-80.006358,-117.1577153,-122.3327673,-122.3899435,-90.1932964,-82.654493,-97.0849471,-97.0849471,-79.3901937,-82.7870177,-78.874097,-77.0082772)
...
c7 <- lm(final~away.rate+home.rate+away.pit.adj+home.pit.adj+doublehead+away.travel+home.travel+away.yest+home.yest+day+div.match+away.wins.20+home.wins.20,mlb.model)
summary(c7)
...
f2 <- lm(away.final~away.rate+home.rate+stadium+home.pit.adj+away.travel+home.travel+away.ball.30+home.ball.30+away.60+home.60,mlb.model)
summary(f2)
n <- predict(f2, mlb.model)
mlb.model$f2 <- n
g2 <- lm(home.final~away.rate+home.rate+stadium+away.pit.adj+away.travel+home.travel+away.ball.30+home.ball.30+away.60+home.60,mlb.model)
summary(g2)
o <- predict(g2, mlb.model)
mlb.model$g2 <- o
mlb.model$e6 <- mlb.model$f2 + mlb.model$g2
mlb.model$e6res <- mlb.model$total - mlb.model$e6
ggplot(mlb.model, aes(e6res)) +
  geom_histogram(binwidth = 1)
#ggplot(mlb.model, aes(e4res,total)) +
#  geom_point()
summary(mlb.model$e6)
sd(mlb.model$e6res)


f3 <- lm(away.final~away.rate+home.rate+stadium+home.pit.adj+away.travel+home.travel+away.ball.30+home.ball.30+away.60+home.60+month+div.match,mlb.model)
summary(f3)
n <- predict(f3, mlb.model)
mlb.model$f3 <- n
g3 <- lm(home.final~away.rate+home.rate+stadium+away.pit.adj+away.travel+home.travel+away.ball.30+home.ball.30+away.60+home.60+month+div.match,mlb.model)
summary(g3)
...
sd3 <- sd(mlb.model$e6res)
today.games$pred.final <- suppressWarnings(predict(c4,today.games))
today.games$away.run <- suppressWarnings(predict(f2,today.games))
today.games$home.run <- suppressWarnings(predict(g2,today.games))
...
today.games %>%
  ...
  mutate(away.prob=pnorm(-pred.final/sd4)-.0275,home.prob=pnorm(pred.final/sd4)+.0275,
         ...
         o=if_else(runs<=6.5,pnorm((pred.run-runs)/sd3)-.026,
                   if_else(runs==7.0,pnorm((pred.run-runs)/sd3)-.024,
                   if_else(runs==7.5,pnorm((pred.run-runs)/sd3)-.044,
                   if_else(runs==8.0,pnorm((pred.run-runs)/sd3)-.035,
                   if_else(runs==8.5,pnorm((pred.run-runs)/sd3)-.046,
                   if_else(runs==9.0,pnorm((pred.run-runs)/sd3)-.064,
                   if_else(runs==9.5,pnorm((pred.run-runs)/sd3)-.059,
                   if_else(runs==10.0,pnorm((pred.run-runs)/sd3)-.066,
                           pnorm((pred.run-runs)/sd3)-.035)))))))),
         u=if_else(runs<=6.5,pnorm(-(pred.run-runs)/sd3)+.026,
                   if_else(runs==7.0,pnorm(-(pred.run-runs)/sd3)+.024,
                   if_else(runs==7.5,pnorm(-(pred.run-runs)/sd3)+.044,
                   if_else(runs==8.0,pnorm(-(pred.run-runs)/sd3)+.035,
                   if_else(runs==8.5,pnorm(-(pred.run-runs)/sd3)+.046,
         ...
         a="|",b="|",c="|") %>%
  select(away=away.team,home=home.team,pred.final,a,away1.5,home1.5,b,runs,over,under,c,away.imp,home.imp)
...
today.games %>%
  ...
  mutate(away.prob=pnorm(-pred.final/sd4)-.0275,home.prob=pnorm(pred.final/sd4)+.0275,
         ...
         a="|",b="|",c="|") %>%
  select(away=away.team,home=home.team,pred.final,a,away.odds,home.odds,b,runs,over,under,c,away.ml,home.ml)
```
...
```
old 140.2339 4580.467 0.0306 142.0729 3460.155 0.0411
new 234.4339 4407.452 0.0532 244.5729 3674.155 0.0666
adj 273.1339 4244.452 0.0644 260.2729 3803.155 0.0684
