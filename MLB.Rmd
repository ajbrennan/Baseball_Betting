...
mlb <- data.frame(game.id = seq(1,nrow(raw)),
                  ...
                  away.15 = as.integer(NA), home.15 = as.integer(NA),
...
for(i in c(1:30)){
  ...
  for(j in c(1:nrow(temp))){
    if(temp$away.team[j] == mlb.teams[i]){
      num <- temp$game.id[j]
      if(j>16)
        mlb$away.15[num] <- mean(y[c((j-15):(j-1))])
      else
        mlb$away.15[num] <- mean(y[c(1:j)])
    }
    if(temp$home.team[j] == mlb.teams[i]){
      num <- temp$game.id[j]
      if(j>16)
        mlb$home.15[num] <- mean(y[c((j-15):(j-1))])
      else
        mlb$home.15[num] <- mean(y[c(1:j)])
    }
  }
}
...
ggplot(mlb.model, aes(final)) +
  geom_histogram(binwidth = 1)
#################
f2 <- lm(away.final~away.rate+home.rate+stadium+home.pit.adj+away.travel+home.travel+away.ball.30+home.ball.30+away.15+home.15,mlb.model)
summary(f2)
n <- predict(f2, mlb.model)
mlb.model$f2 <- n
g2 <- lm(home.final~away.rate+home.rate+stadium+away.pit.adj+away.travel+home.travel+away.ball.30+home.ball.30+away.15+home.15,mlb.model)
summary(g2)
...
today.games %>%
  ...
  mutate(away.prob=pnorm(-pred.final/sd4)-.0275,home.prob=pnorm(pred.final/sd4)+.0275,
         ...
         a="|",b="|",c="|") %>%
  select(away=away.team,home=home.team,pred.final,a,away.spr,home.spr,b,runs,over,under,c,away.ml,home.ml)
```
...
accuracy <- accuracy %>%
  mutate(o=pnorm((pred.run-6.5)/sd3)-.026,u=pnorm(-(pred.run-6.5)/sd3)+.026)
TotalPlot(accuracy,6.5)
accuracy <- accuracy %>%
  mutate(o=pnorm((pred.run-7)/sd3)-.023,u=pnorm(-(pred.run-7)/sd3)+.023) %>%
  filter(total!=7)
TotalPlot(accuracy,7)
accuracy <- accuracy %>%
  mutate(o=pnorm((pred.run-7.5)/sd3)-.044,u=pnorm(-(pred.run-7.5)/sd3)+.044)
TotalPlot(accuracy,7.5)
accuracy <- accuracy %>%
  mutate(o=pnorm((pred.run-8)/sd3)-.035,u=pnorm(-(pred.run-8)/sd3)+.035) %>%
  filter(total!=8)
TotalPlot(accuracy,8)
accuracy <- accuracy %>%
  mutate(o=pnorm((pred.run-8.5)/sd3)-.046,u=pnorm(-(pred.run-8.5)/sd3)+.046)
TotalPlot(accuracy,8.5)
accuracy <- accuracy %>%
  mutate(o=pnorm((pred.run-9)/sd3)-.064,u=pnorm(-(pred.run-9)/sd3)+.064) %>%
  filter(total!=9)
TotalPlot(accuracy,9)
accuracy <- accuracy %>%
  mutate(o=pnorm((pred.run-9.5)/sd3)-.059,u=pnorm(-(pred.run-9.5)/sd3)+.059)
TotalPlot(accuracy,9.5)
accuracy <- accuracy %>%
  mutate(o=pnorm((pred.run-10)/sd3)-.066,u=pnorm(-(pred.run-10)/sd3)+.066) %>%
  filter(total!=10)
TotalPlot(accuracy,10)
accuracy <- accuracy %>%
  mutate(o=pnorm((pred.run-10.5)/sd3)-.035,u=pnorm(-(pred.run-10.5)/sd3)+.035)
...
h <- filter(h,n>1)
sum(abs(h$x-h$p)*h$n)/sum(h$n) #0.01498361 - .038 // 0.0116279 - .033
```
